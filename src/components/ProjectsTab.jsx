    ...
    return (
        <div className="animate-fadeIn">
            {[
                ...
                <div key={idx}>
                    <header className={`border-b-2 border-slate-200 pb-4 mb-8 md:mb-10 ${idx > 0 ? 'md:mt-14' : ''}`}>
                        <h2 className="text-2xl md:text-3xl font-serif text-slate-800 tracking-tight">{section.title}</h2>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 md:gap-16 p-1 md:p-4 mb-10">
                        {section.list.map((project) => (
                            <div key={project.id} className={`relative bg-stone-50 p-3 pb-12 md:pb-16 shadow-lg border border-stone-200 transition-all duration-300 transform ${project.tilt} hover:rotate-0 hover:scale-105 hover:shadow-2xl hover:z-50 group cursor-pointer`}>
                        ...
                        `}
                                ...
                                <a href={project.liveLink} target="_blank" rel="noreferrer" className="block">
                                    <div className="w-full aspect-video bg-slate-800 relative overflow-hidden flex items-center justify-center group-hover:bg-slate-700 transition-colors">
                                        ...
                                        {!project.media || project.media.trim() === '' ? (
                                            <div className="w-full h-full bg-stone-300 flex items-center justify-center relative overflow-hidden">
                                                <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'repeating-linear-gradient(45deg, #000, #000 10px, transparent 10px, transparent 20px)' }}></div>
                                            ...
                                            <video src={project.media} autoPlay loop muted playsInline className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity" />
                                        ) : (
                                            <img src={project.media} alt={project.title} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity" />
                                        )}
                                    ...
                                    <div className="absolute bottom-0 left-0 w-full px-3 md:px-4 py-2 md:py-3 flex justify-between items-end">
                                        <div>
                                            <h3 className="font-serif font-bold text-slate-800 text-xs md:text-lg leading-tight group-hover:text-amber-700 transition-colors">{project.title}</h3>
                                ...
                                <div className="absolute bottom-2 md:bottom-3 right-3 md:right-4 z-30">
                                    <a href={project.githubLink} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-slate-900 transition-colors" title="View Repository" onClick={(e) => e.stopPropagation()}>
                                        <GithubIcon />
                                    </a>
                                </div>
                            </div>
